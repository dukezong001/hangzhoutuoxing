...
static int hp_jack_status_check(void *data)
{
	...
	if (hp_status != priv->hp_active_low) {
		//snd_soc_dapm_disable_pin(dapm, "Ext Spk");//jhf demo
		snd_soc_dapm_enable_pin(dapm, "Ext Spk");// jhf
		if (priv->is_headset_jack) {
			//snd_soc_dapm_enable_pin(dapm, "Mic Jack");
			//snd_soc_dapm_disable_pin(dapm, "Main MIC");
		   snd_soc_dapm_disable_pin(dapm, "Mic Jack");
		   snd_soc_dapm_enable_pin(dapm, "Main MIC"); 
			printk("0000000000000000000000000000022222200000\n");
		}
		...
		printk("2222222222222222222222222222222222222222222222\n");
	}
...
static int mic_jack_status_check(void *data)
{
	...
	if (mic_status != priv->mic_active_low) {
		...
		printk("333333333333333333333333333333333333333333333\n");
	} else {
		snd_soc_dapm_enable_pin(dapm, "Main MIC");
		ret = 0;
...
static int imx_wm8960_late_probe(struct snd_soc_card *card)
{
    ...
    //snd_soc_update_bits(codec, WM8960_POWER2, 3<<3, 3);
	//snd_soc_update_bits(codec, WM8960_CLASSD1, 3<<6, 3);
	return 0;
}
